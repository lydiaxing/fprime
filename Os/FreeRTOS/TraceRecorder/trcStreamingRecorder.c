...
#define PSF_ASSERT(_assert, _err, _return) if (! (_assert)){ prvTraceError(_err); return _return; }
...
traceString xTraceRegisterString(const char* name)
{
	...
	PSF_ASSERT(eventID < 4096, PSF_ERROR_EVENT_CODE_TOO_LARGE, (traceString)name);
...
traceHandle xTraceSetISRProperties(const char* name, uint8_t priority)
{
	...
	PSF_ASSERT(eventID < 4096, PSF_ERROR_EVENT_CODE_TOO_LARGE, (traceHandle)name);
...
static void prvTraceStoreWarnings(void)
{
	if (RecorderEnabled)
	{
		...
		if (errStr != NULL)
		{
			prvTraceStoreSimpleStringEventHelper(PSF_EVENT_USER_EVENT, trcWarningChannel, errStr);
		}
	}
}
